...
{
    hashcrypt_hash_ctx_t sha2_state;
    shaX_inc_init(&sha2_state);
    unsigned char buf[SPX_SHA256_ADDR_BYTES + SPX_N];
    unsigned char outbuf[SPX_SHA256_OUTPUT_BYTES];

    /* Retrieve precomputed state containing pub_seed */
    // memcpy(sha2_state.x, ctx->state_seeded, 40 * sizeof(uint8_t));
    sha256_inc_blocks(&sha2_state, ctx->state_seeded, 1);
